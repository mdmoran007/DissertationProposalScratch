...
LettersList = []
OutList = [['LetterFileName', 'Ticker', 'Fiscal_Year', 'Eco']]
...
#        data = data_temp.encode("ascii", "ignore")
        ...
        for i in range(0,len(words)):
            if ("ecosystem" in words[i].lower()):
                print(fn, fn.split('-')[0], fn.split('-')[2][2:6], words[i], 'eco')
                if i > 5:
                    print(words[i-5:i+1])
                OutList.append([fn, fn.split('-')[0], fn.split('-')[2][2:6], '1'])

OutFile = '/Users/moranmarkd/OneDrive/Academics/Illinois/Dissertation/Paper1.1/Out/eco-v1.csv'
